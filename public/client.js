...
$.fn.postTea = function(){
    $('#display').append('<form method="post" enctype="multipart/form-data" id="myForm" name="myForm">\
    ...
    $('#newTea').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#display').html("<p>A new tea is born!</p><pre>"+JSON.stringify(data,null,2)+"</pre>")
            },
            error: function (error) {
...
$.fn.delAll = function(){
    $('#display').append('<input type="text" id="apikeydelAll" name="apikeydelAll" placeholder="Admin Password">\
